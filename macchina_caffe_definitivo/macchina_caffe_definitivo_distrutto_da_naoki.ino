...
/* Definisco la grandezza di un array chiamato testo, che conterrà tutte le frasi 
 * in italiano e in inglese, che verranno stampate a video
 */
#define SIZE_TESTO 13                                           
                                                                
//Definisco i prezzi delle bevande
#define PREZZO_CAFFE 1.20                                       
#define PREZZO_THE 1.10

//Definisco  il valore di ogni moneta
#define FR1 1                                                   
#define CT50 0.50
#define CT20 0.20
#define CT10 0.10

//Definisco un valore tempo che verrà applicato facendo lampeggiare un led  
#define TIME_LED_BLINK 1000                                     
//Definisco un valore tempo per cui un led rimarrà acceso o spento
#define TIME_LED_ON 4000                                        
//Definisco un valore tempo che verrà applicato facendo "lampeggiare" delle scritte
#define TIME_VIEWING_TESTO 2000                                 

//Definisco un valore tempo che l'arduino utilizzerà (vedi utilizzo alla riga 170)
#define TIMEOUT_PREZZI 10000                                    
//Definisco un valore tempo che l'arduino utilizzerà (vedi utilizzo lla riga 181)
#define TIMEOUT_MONETA 10000                                    
...
/* Creo delle variabili (tempo_attuale, stato_precedente_A,
 * stato e i)
 ...
 * "cadere" il programma
 * Infine la variabile i serve in due funzioni for (vedi funzionamento alla 
 * righe 163 e 168
 */

int tempo_attuale, stato_precedente_A, stato, i;                

/* Vengono create delle variabili di tipo testo, e altri tre array di testo                                                                                                               
 * Tutti questi variabili e array sono fondamentali per il cambio della
 * lingua oppure per le srcitte da stampare a video (per la funzione cambio
 * lingua guardare alle righe 220 e 226)
 */
                                                                
char* lingua_attuale = "italiano";
char* testo[SIZE_TESTO];
char* italiano[] =
{                                                               //
...
/* Da notare che le frasi sono nello stesso ordine sia in italiano che in inglese,
...
char* english[] =                                               
{
  ...
  "You need insert ",
  "Now you can take your change of ",
  "Goodbye",
};

/* Vengono create delle variabili di tipo float, ovvero credito (a cui verrà assegnato 
 * il valore del credito appunto), moneta_mancante (a cui verrà assegnato, dopo un breve
 * calcolo, il valore della moneta da inserire se questa è insufficiente) e prezzo (che
 * contiene solamente i valori dei costi delle bevande)
 */

float credito, moneta_mancante, prezzo;                         
...
void setup() {
  ...
  //Appunto: stato 0 = Mostra scritta iniziale
  stato = 0;                                                       
  credito = 0;
  moneta_mancante = 0;
  prezzo = 0;
  ...
  tempo_attuale = 0;
  //In questo modo la lungua iniziale sarà l'italiano
    ...
    switch (PINA) {
      ...
      case 0b00000001: /*tasto caffe*/
         ...
         if (credito == 0) {
          ...
          //Appunto: viene mostrata la scritta : Il caffe costa * Fr.
          Serial.print(testo[2]);                               
          Serial.print(prezzo);
          Serial.println("Fr.");
          stato = 5;                                            //Appunto: stato 2 = TIMEOUT PREZZI
        } else {
